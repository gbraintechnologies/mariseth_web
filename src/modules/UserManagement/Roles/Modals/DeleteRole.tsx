...
import { useAccountsUsersGroupsDelete} from "@/apis/adminApiComponents";
import { toast } from "sonner";
import { getErrorMap } from "@/lib/helpers";

const DeleteRoleModal = ({open, onClose, data, refetch, setActiveTab}:
    {open: boolean, 
        onClose: () => void;
        data: any;
        refetch: () => void;
    ...
    const {mutate, isPending} = useAccountsUsersGroupsDelete(
        {
            onSuccess: () => {
       ...
       {/* <ClickOutside onClick={onClose}> */}
         <div className="w-full max-w-[550px] rounded-[15px] bg-white px-8 py-12 text-center shadow-3 dark:bg-gray-dark dark:shadow-card md:px-15 md:py-15">
           ...
           <div className="-mx-2.5 flex flex-wrap gap-y-4">
             ...
             <div className="w-full px-3 2xsm:w-1/2" onClick={handleDelete}>
