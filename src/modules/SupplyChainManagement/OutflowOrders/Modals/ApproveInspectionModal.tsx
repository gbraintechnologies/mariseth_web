...
export default function ApproveInspectionModal({
    open, 
    setOpen, 
    data,
    refetch}:{
        ...
        <Dialog open={open}>
            <DialogContent className="sm:max-w-[500px] p-0 text-[#334155] !rounded-b-lg">
                <DialogTitle className="mt-5 flex justify-between px-5">
                    <div className="font-medium text-[#0F172A]">Approve Delivery Inspection Inbound Order?</div>
                    <XCircle className="text-red-500 cursor-pointer" onClick={() => setOpen(false)}/>
                </DialogTitle>
                <hr/>
                <div className="mt-1 p-5 text-center">
                    Are you sure you want to approve Delivery Inspection Inbound Order ?
                </div>
