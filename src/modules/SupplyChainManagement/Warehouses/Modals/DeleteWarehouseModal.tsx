import { useWarehouseDelete } from "@/apis/adminApiComponents";
...
export default function DeleteWarehouseModal({
    ...
    const {mutate, isPending} = useWarehouseDelete({
        onSuccess: () =>{
            refetch()
            setOpen(false)
        ...
        <Dialog open={open}>
            <DialogContent className="sm:max-w-[650px] p-0 text-[#334155] !rounded-b-lg">
                <DialogTitle className="mt-5 flex justify-between px-5">
                    <div className="font-medium text-[#0F172A]">Delete this Warehouse?</div>
                    <XCircle className="text-red-500 cursor-pointer" onClick={() => setOpen(false)}/>
                </DialogTitle>
                <hr/>
                <div className="mt-1 p-5 text-center">
                    Deleting this warehouse will remove it from the system permanently. <br/>Do you want to proceed?
                </div>
