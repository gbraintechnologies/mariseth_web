"use client"
import {useCustomTypeList, useInflowApproveDeliveryInspection} from "@/apis/adminApiComponents";
...
import {  PlusSquare, Trash2, XCircle } from "lucide-react";
...
export default function ApproveInflowCommentModal({
    open, 
    setOpen, 
    data,
    refetch}:{
    ...
    const getExpectedQuantity = (id: number) => {
        ...
        const qty = data?.products?.find((item: any) => Number(item?.id) === Number(selected_product_id))
                                    ...
                                    {products.map((item, idx) => (
                                        <div key={idx} className=" rounded-lg border p-4 bg-[#F8FAFC] relative">
                                           ...
                                           <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                                ...
                                                <div className="w-full space-y-3">
                                                    <Label className="text-sm">No. of Problematic Bags</Label>
                                                    <div>
                                                        <Input  
                                                            className="" 
                                                            placeholder="0" 
                                                            ...
                                                            onChange={(e) => updateProduct(item.id, 'problematic_quantity', Number(e.target.value))}
                                                            required/>
                                                    </div>
                                                </div>
                                                <div className="w-full space-y-3">
                                            ...
                                            <div className="grid grid-cols-1 w-full space-y-3">
                                                ...
                                                <div>
                                                    <Input  
                                                        ...
                                                        value={item.comment}
                                                        onChange={(e) => updateProduct(item.id, 'comment', e.target.value)}
                                                        />
                                                </div>
                                            </div>
                                        </div>
