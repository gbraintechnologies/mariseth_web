import { permType, useHasAccess } from '@/hooks/auth/useHasAccess';
import React, {ReactNode} from 'react'
import { SuspenseLogo } from './SuspenseLoader';

 export default function Unauthorized(){
    return (
        <div className='rounded-[10px] bg-white shadow-1 dark:bg-gray-dark dark:shadow-card mt-10 p-10'>
                <div className='flex items-center justify-center mb-4 '>
                    <div className="grid grid-cols-1">
                        <div className="flex items-center justify-center mb-10">
                            <svg fill="none" xmlns="http://www.w3.org/2000/svg" width="150" height="150" viewBox="0 0 24 24" className="text-red-500">
                                <path fillRule="evenodd" clipRule="evenodd" d="M19.2013 4.32867C19.9461 4.57382 20.4493 5.26122 20.4493 6.02608L20.4997 12.6637C20.5097 14.674 19.775 16.6253 18.4263 18.1561C17.8123 18.8621 17.0172 19.4603 16.0107 19.9996L12.4377 21.9117C12.327 21.9706 12.2062 22 12.0855 22C11.9647 22 11.8338 21.9706 11.7231 21.9117L8.11991 20.0486C7.10336 19.5191 6.30824 18.9307 5.68422 18.2345C4.3154 16.7244 3.55047 14.773 3.54041 12.7628L3.50015 6.12316C3.49008 5.3583 3.99333 4.67286 4.72806 4.41693L11.3407 2.1037C11.7332 1.96543 12.166 1.96543 12.5686 2.1037L19.2013 4.32867ZM14.4205 14.0866C14.7124 13.8022 14.7124 13.3315 14.4205 13.0472L13.0617 11.7224L14.4205 10.3995C14.7124 10.1152 14.7124 9.65429 14.4205 9.35913C14.1286 9.07476 13.6455 9.07476 13.3536 9.35913L11.9949 10.6839L10.6361 9.35913C10.3442 9.07476 9.87119 9.07476 9.56924 9.35913C9.27736 9.65429 9.27736 10.1152 9.56924 10.3995L10.928 11.7224L9.56924 13.0472C9.27736 13.3315 9.27736 13.8022 9.56924 14.0866C9.72022 14.2337 9.91145 14.3013 10.1027 14.3013C10.304 14.3013 10.4952 14.2337 10.6361 14.0866L11.9949 12.7628L13.3536 14.0866C13.5046 14.2337 13.6958 14.3013 13.8871 14.3013C14.0783 14.3013 14.2796 14.2337 14.4205 14.0866Z" fill="currentColor" />
                            </svg>
                        </div>
                        <div className="">
                            <p className="mb-0 mt-8 text-xl text-center font-bold text-[#64748B]">
                            You do not have permission to <br/>access this feature
                            </p>
                        </div>
                    </div>
                </div>
        </div>
    )
}

export function AuthorizeAndRenderPage({permission, children}: {permission: permType, children: ReactNode}) {
    const {hasAccess, loading} = useHasAccess(permission);
    if (loading) {
        return <><SuspenseLogo/></>;
    }
    if (hasAccess) {
        return children;
    }
    return <Unauthorized />
}
