import { useHelpDelete } from "@/apis/adminApiComponents";
import { CustomType } from "@/apis/adminApiSchemas";
...
export default function DeleteHelpModal({
    open, 
    setOpen, 
    data,
    refetch
    ...
    const {mutate, isPending} = useHelpDelete({
        onSuccess: () =>{
            refetch()
        ...
        <Dialog open={open}>
            <DialogContent className="sm:max-w-[500px] p-0 text-[#334155] !rounded-b-lg">
                <DialogTitle className="mt-5 flex justify-between px-5">
                    <div className="font-medium text-[#0F172A]">Delete this help?</div>
                    <XCircle className="text-red-500 cursor-pointer" onClick={() => setOpen(false)}/>
                </DialogTitle>
                <hr/>
                <div className="mt-1 p-5 text-center">
                    Deleting this will remove it from the system permanently. <br/>Do you want to proceed?
                </div>
                <DialogFooter className="flex !justify-between p-5 bg-[#F8FAFC]">
                    ...
                    <Button onClick={handleDelete} type="button" variant={"destructive"}>
                        <LoadingLabel isLoading={isPending}>Yes, Delete</LoadingLabel>
                    </Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
