import { useLeaveTypeDelete } from "@/apis/adminApiComponents";
...
export default function DeleteLeaveTypeModal({
    ...
    const {mutate, isPending} = useLeaveTypeDelete({
        onSuccess: () =>{
            refetch()
            setOpen(false)
        ...
        <Dialog open={open}>
            <DialogContent className="sm:max-w-[500px] p-0 text-[#334155] !rounded-b-lg">
                <DialogTitle className="mt-5 flex justify-between px-5">
                    <div className="font-medium text-[#0F172A]">Delete Leave Type?</div>
                    <XCircle className="text-red-500 cursor-pointer" onClick={() => setOpen(false)}/>
                </DialogTitle>
                <hr/>
                <div className="mt-1 p-5 text-center">
                    Are you sure you want to delete leave type <span className="font-bold">{data?.name}</span>?
                </div>
