import { useTrainingDelete } from "@/apis/adminApiComponents";
...
export default function DeleteTrainingModal({
    ...
    const {mutate, isPending} = useTrainingDelete({
        onSuccess: () =>{
            refetch()
            setOpen(false)
        ...
        <Dialog open={open}>
            <DialogContent className="sm:max-w-[500px] p-0 text-[#334155] !rounded-b-lg">
                <DialogTitle className="mt-5 flex justify-between px-5">
                    <div className="font-medium text-[#0F172A]">Are you sure you want to delete this training?</div>
                    <XCircle className="text-red-500 cursor-pointer" onClick={() => setOpen(false)}/>
                </DialogTitle>
                <hr/>
                <div className="mt-1 p-5 text-center">
                    Deleting this training is irreversible. Do you want to proceed?  <span className="font-bold">{data?.name}</span>?
                </div>
