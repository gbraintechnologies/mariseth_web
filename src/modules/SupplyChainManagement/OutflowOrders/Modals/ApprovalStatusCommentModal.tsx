"use client"
import {useCustomTypeList, useOutflowMarkComplete} from "@/apis/adminApiComponents";
...
export default function ApprovalStatusCommentModal({
    open, 
    setOpen, 
    data,
    refetch}:{
    ...
    const getExpectedQuantity = (id: number) => {
        const selected_product_id = products?.find((item) => item._id === id)?.product
        const qty = data?.products?.find((item: any) => Number(item?.product?.id) === Number(selected_product_id))
        const expected_quantity = Number(qty?.expected_quantity || 0).toFixed(0)
    ...
    const updateProduct = (id: number, field: string, value: string) => {
        ...
        if(field === "product"){
            const qty = data?.products?.find((item: any) => Number(item?.product?.id) === Number(value))
            const expected_quantity = Number(qty?.expected_quantity || 0).toFixed(0)
            setProducts(products.map(product => 
                product._id === id ? { ...product, ["product"]: value, ["expected_quantity"]: expected_quantity } : product
            ));
        }else{
    ...
    const {mutate, isPending} = useOutflowMarkComplete({
            onSuccess: () =>{
                refetch()
        ...
        <Dialog open={open}>
            <DialogContent className="md:max-w-[980px] p-0 text-[#334155] !rounded-b-lg">
                <DialogTitle className="mt-5 flex justify-between px-5">
                    <div className="font-medium text-[#0F172A]">Recipient Comments</div>
                    ...
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 container md:w-2/3s mx-auto ">
                        <div className="h-[500px] overflow-y-auto">
                            ...
                            <div className="gap-5s">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-5 ">
                                           ...
                                           <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                                <div className="w-full space-y-3">
                                                    ...
                                                    <Select
                                                        required
                                                        value={item.product}
                                                        onValueChange={(value) => updateProduct(item._id, 'product', value)}
                                                    >
                                                ...
                                                <div className="w-full space-y-3">
                                                    <Label className="text-sm">No. of Problematic Bags<div className='text-red-500'>*</div></Label>
                                                    <div>
                                                        <Input  
                                                            className="" 
                                                            placeholder="0" 
                                                            ...
                                                            onChange={(e) => updateProduct(item._id, 'problematic_quantity', e.target.value)}
                                                            required/>
                                                    </div>
                                                </div>
                                                <div className="w-full space-y-3">
                                    ...
                                    <div className="p-1">
                                    <div onClick={addNewProductField} className="bg-[#cbd5e133]  h-full  rounded-lg outline-2 outline-offset-2 outline-dashed outline-[#CBD5E1] flex items-center justify-center cursor-pointer hover:bg-[#cbd5e163] transition-all duration-200 ease-in-out">
                        ...
                        <div className="flex justify-center">
                            <Button type="submit" className="bg-[#16A34A] text-white w-fulls rounded-md cursor-pointer"> 
                                <LoadingLabel isLoading={isPending}>Submit Recipient Comments</LoadingLabel>
                            </Button>
                        </div>
