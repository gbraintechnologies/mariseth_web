...
import { countUniqueFarms } from "../utils/helpers";
...
export default function PrintWaybillInbound({data}:{data: any}){
            ...
            <div className="max-w-5xl mx-auto bg-white p-5 px-8 shadow-sms border border-gray-200 relative">
                ...
                <div className="grid grid-cols-12 mt-8 mb-5">
                    <div className="col-span-4">
                        <h2 className="font-semibold uppercase mb-2">INBOUND WAYBILL N0: </h2>
                    ...
                    <div className="col-span-8 text-gray-600">
                        ...
                        <div className="text-sm">{formatDateReadable(data?.order_creation_date)}</div>
                    </div>
                ...
                <div className="border-r border-b border-[#000000] pb-5">
                    <h2 className="font-semibold uppercase mb-2 mt-5">Aggregation Details</h2>
                    <div className="text-gray-600 text-sm uppercase space-y-2">
                        <p className="grid grid-cols-2 gap-8"><span className="font-normal ">Aggregator Name:</span> {data?.aggregator?.first_name} {data?.aggregator?.last_name}</p>
                        <p className="grid grid-cols-2 gap-8"><span className="font-normal">Aggregator Number:</span> {data?.aggregator?.phone_number}</p>
                        <p className="grid grid-cols-2 gap-8"><span className="font-normal">Procurement Officer:</span> {data?.procurement_officer?.first_name} {data?.procurement_officer?.last_name}</p>
                        <p className="grid grid-cols-2 gap-8"><span className="font-normal">Aggregated Farms:</span> {countUniqueFarms(data?.products)}</p>
                    </div>
                </div>
                <div className="border-b px-5 border-[#000000] pb-5">
                    <h2 className="font-semibold uppercase mb-2 mt-5">Receiving Warehouse</h2>
                    <div className="text-gray-600 text-sm uppercase space-y-2">
                        <p className="grid grid-cols-2 gap-8"><span className="font-normal">Warehouse Name:</span> {data?.destination_warehouse?.name}</p>
                        {/* <p className="grid grid-cols-2 gap-8"><span className="font-normal">Warehouse Address:</span> Solaga, BR</p> */}
                        <p className="grid grid-cols-2 gap-8"><span className="font-normal">Warehouse Manager:</span> {data?.destination_warehouse?.warehouse_managers?.[0]?.first_name} {data?.destination_warehouse?.warehouse_managers?.[0]?.last_name}</p>
                        <p className="grid grid-cols-2 gap-8"><span className="font-normal">Manager No.:</span> {data?.destination_warehouse?.warehouse_managers?.[0]?.phone_number}</p>
                    </div>
                </div>
                </div>
                    ...
                    <tbody className="text-gray-600">
                        {data?.products?.map((item: any, idx: number) => (
                            <tr className="border-bs text-gray-600" key={idx}>
                                ...
                                <td className="py-5 px-2 text-gray-600">{item?.farm?.name}</td>
                                <td className="py-5 px-2 text-gray-600">{commaSeparator(item?.quantity)}</td>
                                <td className="py-5 px-2 text-gray-600">{commaSeparator(item?.total_weight || 0)} KG</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
                ...
                <div className="text-right font-normal mb-8 text-gray-600 mt-5">
                <span className="text-gray-600">Total Weight:</span> <span className="text-black font-medium">{commaSeparator(data?.total_weight || 0)} KG</span>
                </div>
